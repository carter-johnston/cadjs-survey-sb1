<script>
	...
	function sortByActive() {
		if (sortByActiveOn == true) fullSurveyList.sort((a, b) => b.isActive - a.isActive);
		else fullSurveyList.sort((a, b) => a.isActive - b.isActive);
	}

	function sortByNewest() {
		if (sortByNewestOn == true) {
			fullSurveyList.sort((a, b) => (b.creationDate > a.creationDate ? 1 : -1));
		} else {
			fullSurveyList.sort((a, b) => (b.creationDate < a.creationDate ? 1 : -1));
		}
	}
	...
	function sortSurveys() {
		sortByNewest();
		sortByActive();
		pagedSurveyList = fullSurveyList.slice(pageIndex * pageLength, (pageIndex + 1) * pageLength);
	}
...
<div class="ms-5 me-5 mt-2">
	...
	<div class="d-flex">
		...
		{:else}
			<button class="btn btn-success me-2" on:click="{() => (sortByActiveOn = true)}"
				>Sort by Active</button>
		{/if}
		{#if sortByNewestOn == true}
			<button class="btn btn-secondary me-2" on:click="{() => (sortByNewestOn = false)}"
				>Disable Sort by Newest</button>
		{:else}
			<button class="btn btn-success me-2" on:click="{() => (sortByNewestOn = true)}"
